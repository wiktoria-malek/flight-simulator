...
class KnobSystem:
    def __init__(self, lattice, p_ref=-1300):
        ...
        self.linear_matrix = {
            "A_x": {"SD0FF": (-116.5, 0), "SF1FF": (-35.2, 0), "SD4FF": (-37.8, 0), "SF5FF": (0, 0),
                    "SF6FF": (-623.4, 0)},
            "E_x": {"SD0FF": (-813.0, 0), "SF1FF": (793.5, 0), "SD4FF": (-137.9, 0), "SF5FF": (0, 0),
                    "SF6FF": (1492.9, 0)},
            "A_y": {"SD0FF": (-98.9, 0), "SF1FF": (-9.6, 0), "SD4FF": (-246.7, 0), "SF5FF": (0, 0),
                    "SF6FF": (120.0, 0)},
            "E_y": {"SD0FF": (0, 374.1), "SF1FF": (0, -120.0), "SD4FF": (0, -451.3), "SF5FF": (0, 0),
                    "SF6FF": (0, -64.3)},
        ...
        for knob, mapping in self.nonlinear_matrix.items():
            ...
            for base, val in mapping.items():
                ...
                if isinstance(val, (int, float)):
                    ...
                    coeff_dK = float(val[0]);
                    order = int(val[1]) if len(val) > 1 else 2;
                    comp = str(val[2]) if len(val) > 2 else 'normal'
                elif isinstance(val, dict):
                    coeff_dK = float(val.get('coeff', 0.0));
                    order = int(val.get('order', 2));
                    comp = str(val.get('comp', 'normal'))
                else:
