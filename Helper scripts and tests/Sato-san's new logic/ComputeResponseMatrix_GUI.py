...
class MainWindow(QMainWindow):
    def __init__(self, data_dir=None):
        ...
        self.cwd = os.path.abspath(data_dir) if data_dir else os.getcwd()
        self.datafiles = sorted(glob.glob(os.path.join(self.cwd, 'DATA*.pkl')))
        self.pair_re = re.compile(r"DATA_(.+)_(p|m)(\d+)\.pkl$")
        if not self.datafiles:
            chosen = QFileDialog.getExistingDirectory(
                self, "Select data directory", os.path.expanduser("~/flight-simulator-data/")
            )
            if chosen:
                self.cwd = chosen
                self.datafiles = sorted(glob.glob(os.path.join(self.cwd, 'DATA*.pkl')))
        ...
        if not self.datafiles:
            ...
            self.statusBar().showMessage("No DATA*.pkl found. Please restart from a valid data folder.")
    ...
    def __compute_button_clicked(self):
        ...
        for f in self.datafiles:
            m = self.pair_re.search(os.path.basename(f))
            if m:
                tags.append(m.group(1))
        is_qm_mode = any(tag.endswith("_x") or tag.endswith("_y") for tag in tags)
        ...
        if is_qm_mode:
            hcorrs = sorted({tag[:-2] for tag in tags if tag.endswith("_x")}, key=seq_index)
            vcorrs = sorted({tag[:-2] for tag in tags if tag.endswith("_y")}, key=seq_index)
        else:
        ...
        for f in self.datafiles:
            ...
            if m and m.group(2) == "p":
                datafiles_p.append(f)
        for datafile_p in datafiles_p:
            m = self.pair_re.search(os.path.basename(datafile_p))
            if not m:
                continue
            datafile_m = os.path.join(os.path.dirname(datafile_p), f"DATA_{m.group(1)}_m{m.group(3)}.pkl")
            if os.path.exists(datafile_m):
                ...
                if is_qm_mode:
                    qxp = Sp.get_quadrupoles(hcorrs)
                    qyp = Sp.get_quadrupoles(vcorrs)
                    qxm = Sm.get_quadrupoles(hcorrs)
                    qym = Sm.get_quadrupoles(vcorrs)
                    Cx_p = qxp.get('xact', qxp.get('xdes', np.zeros(len(hcorrs))))
                    Cy_p = qyp.get('yact', qyp.get('ydes', np.zeros(len(vcorrs))))
                    Cx_m = qxm.get('xact', qxm.get('xdes', np.zeros(len(hcorrs))))
                    Cy_m = qym.get('yact', qym.get('ydes', np.zeros(len(vcorrs))))
                else:
        ...
        if self.triangular_checkbox.isChecked():
            for corr in hcorrs:
                 bpm_indexes = [ bpms.index(bpm) for bpm in bpms if seq_index(bpm) < seq_index(corr) ]
...
window = MainWindow(data_dir=start_dir)
window.show()
sys.exit(app.exec())
