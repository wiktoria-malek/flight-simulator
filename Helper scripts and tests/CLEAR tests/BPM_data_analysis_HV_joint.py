...
M_v = np.array([
    [next(result["V"] for result in all_results if result["bpm"] == bpm and result["file"]==filename)
                 for bpm in bpms]
                for filename in list_of_files])
M_raw = np.hstack((M_h, M_v))
M = M_raw - M_raw.mean(axis=0, keepdims=True)
channel_labels = [f"{bpm} H" for bpm in bpms] + [f"{bpm} V" for bpm in bpms]

U, singular_values, Vt = np.linalg.svd(M, full_matrices=False)
variance_percent = singular_values**2 / np.sum(singular_values**2) * 100

print("Joint H+V singular values:", singular_values)
print("Joint H+V explained variance [%]:", variance_percent)
print("First joint mode:", Vt[0])
print("Second joint mode:", Vt[1])

for coordinate, matrix in (("H", M_h), ("V", M_v)):
    plt.figure()
...
ax.set_title(r"Joint matrix with data $M = [M_H\;M_V]$")
...
std_h = np.std(M[:, :len(bpms)], axis=0)
std_v = np.std(M[:, len(bpms):], axis=0)
plt.figure()
...
plt.xlabel("BPM")
plt.ylabel("Std [mV]")
plt.title("Standard deviation of joint-matrix M (H+V)")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
ax.set_title(r"$U$ from joint H+V SVD")
...
ax.set_title(r"$V^T$ from joint H+V SVD")
...
plt.title(r"First two modes $V^T$ of joint SVD")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
plt.title("Singular values of joint H+V matrix")
plt.xticks(modes)
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.xlabel("SVD mode")
plt.ylabel("Variance [%]")
plt.title("Variance of joint H+V SVD")
plt.xticks(modes)
plt.grid(axis="y")
plt.tight_layout()
plt.show()
...
plt.figure()
for bpm in bpms:
    s_values = np.asarray([result["S"] for result in all_results], dtype=float)
    h_values = [result["H"] for result in all_results]
