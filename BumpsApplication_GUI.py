...
try:
    ...
    from PyQt6.QtWidgets import (QApplication, QCheckBox, QDialog, QDialogButtonBox, QDoubleSpinBox, QFileDialog, QFormLayout, QHeaderView, QLabel, QMainWindow, QMessageBox, QPushButton, QSizePolicy, QVBoxLayout, QWidget, QTableWidgetItem)
    pyqt_version = 6
except ImportError:
    from PyQt5 import uic
    from PyQt5.QtCore import Qt, QProcess, QProcessEnvironment, QTimer
    from PyQt5.QtGui import QPixmap
    from PyQt5.QtWidgets import (QApplication, QCheckBox, QDialog, QDialogButtonBox, QDoubleSpinBox, QFileDialog, QFormLayout, QHeaderView, QLabel, QMainWindow, QMessageBox, QPushButton, QSizePolicy, QVBoxLayout, QWidget)
    pyqt_version = 5
...
class MainWindow(QMainWindow, SaveOrLoad, ResponseMatrix_DFS_WFS):
    ...
    def _wire_signals(self):
        ...
        self.response_dir_edit.returnPressed.connect(lambda: self._load_response_directory(self.response_dir_edit.text()))
        self.save_correctors_button.clicked.connect(lambda: self._saving_func(self.correctors_list, "bump_correctors.txt", "Save Correctors"))
        ...
        self.desired_plot.mpl_connect("button_press_event", lambda event: self._handle_plot_double_click(event, "desired"))
        self.result_plot.mpl_connect("button_press_event", lambda event: self._handle_plot_double_click(event, "result"))
    ...
    def _open_plot_popup(self, plot_name):
        ...
        if popup is None:
            ...
            if plot_name == "desired":
                ...
                popup.plot.mpl_connect("button_press_event", lambda event: self._handle_plot_double_click(event, "desired"))
        popup.show()
        if plot_name == "desired":
            self._refresh_desired_plot()
        else:
    ...
    def _draw_desired_plot(self, canvas, bpms):
        ...
        lines = {}
        for axis, reference, target, plane in ((canvas.ax_x, reference_x, target_x, "x"), (canvas.ax_y, reference_y, target_y, "y")):
    ...
    def _compute_orbit_bump(self, correctors_currents):
        ...
        current_corrector_settings = np.asarray(self.interface.get_correctors(self.corrector_names)["bact"], dtype=float)
    ...
    def _restore_reference(self):
        ...
        if self.interface.restore_correctors_state(self.restore_state) is False:
            ...
            QMessageBox.warning(self, "Restore initial settings","Some correctors were not confirmed back at their saved current within the readback tolerance. Check them on the machine before the next correction.")
